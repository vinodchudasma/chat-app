...
export default function GroupCall({
  ...
  useEffect(() => {
    ...
    socket.on(
      "group_call_participants_with_info",
      handleGroupCallParticipantsWithInfo,
    );
    ...
    return () => {
      ...
      socket.off(
        "receive_group_call_ice_candidate",
    ...
    if (screenShareVideoRef.current) {
      ...
      screenShareVideoRef.current.onloadedmetadata = () => {
        screenShareVideoRef.current
          .play()
          .catch((e) => console.log("Screen share video play error:", e));
      };
    }
  ...
  const handleIncomingOffer = async (
    ...
    try {
      ...
      if (!mediaStreamRef.current && !isScreenShare) {
        ...
        pendingOffers.current[callerId].push({
          offer,
          callerSocketId,
          isScreenShare,
        });
    ...
    Object.keys(pendingOffers.current).forEach((callerId) => {
      ...
      offers.forEach(async (offerData) => {
        await handleIncomingOffer(
          callerId,
          offerData.offer,
          offerData.callerSocketId,
          offerData.isScreenShare,
        );
      });
    });
    pendingOffers.current = {};
  ...
  const handleEndCall = () => {
    ...
    if (socket) {
      const eventName =
        callData.type === "video"
          ? "group_video_call_end"
          : "group_voice_call_end";
            ...
            <div className="flex items-center space-x-3">
              ...
              <div>
                ...
                <p className="text-gray-400 text-sm flex items-center">
                  ...
                  Incoming {callData.type === "video" ? "Video" : "Voice"} Group
                  Call
                </p>
              </div>
            </div>
        ...
        <div className="max-w-6xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
            ...
            <div className="bg-gray-900/50 rounded-2xl border border-gray-800 overflow-hidden h-64">
              <div className="p-3 border-b border-gray-800">
                <div className="flex items-center justify-between">
                  <div className="flex items-center space-x-2">
                    <div
                      className={`w-2 h-2 rounded-full ${localStream ? "bg-green-500" : "bg-yellow-500"}`}
                    />
            ...
            {participants.map((userId) => (
              <div
                key={userId}
                className="bg-gray-900/50 rounded-2xl border border-gray-800 overflow-hidden h-64"
              >
                <div className="p-3 border-b border-gray-800">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center space-x-2">
                      <div
                        className={`w-2 h-2 rounded-full ${participantStreams[userId] ? "bg-green-500" : "bg-yellow-500"}`}
                      />
          ...
          <div className="flex flex-col lg:flex-row gap-6">
            ...
            <div className="lg:w-64 bg-gray-900/50 rounded-2xl border border-gray-800 p-4">
              ...
              <div className="space-y-3">
                ...
                <div className="flex justify-between items-center">
                  ...
                  <span
                    className={`${callData.type === "video" ? "text-blue-400" : "text-green-400"}`}
                  >
                    {callData.type === "video"
                      ? "🎥 Video Group"
                      : "🔊 Voice Group"}
            ...
            <div className="flex-1">
              <div className="flex justify-center items-center space-x-6">
                ...
                <button
                  onClick={
                    isSharingScreen ? stopScreenSharing : startScreenSharing
                  }
                  className={`w-14 h-14 rounded-full flex items-center justify-center transition-all ${
                    isSharingScreen
